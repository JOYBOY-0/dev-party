    ...
    <div className='w-screen fixed top-0 z-50 bg-gradient-to-b from-slate-900 to-slate-900/0'>
      <div className='w-full mx-auto max-w-[1400px] flex justify-between items-center p-6'>
        <Link to={PublicRoutes.HOME} className='h-full flex items-center'>
          <img
            src='/logo/logo_sm_color.svg'
            alt='logo'
            className='h-[45px] md:hidden my-auto aspect-square'
          />
          <img
            src='/logo/logo_md_color.svg'
            alt='logo'
            className='h-4 hidden md:flex lg:hidden'
          />
          <img
            src='/logo/logo_lg_color.svg'
            alt='logo'
        ...
        <div className='flex items-center h-full '>
          <div className='hidden lg:flex items-center h-full space-x-6 font-primary mr-20'>
            ...
            <NavLink
              to={PrivateRoutes.COLLECTION}
              className={({ isActive }: { isActive: boolean }) =>
                isActive
                  ? 'text-devBlue-600 font-medium underline'
                  : 'text-slate-200 hover:text-white'
              }
            >
              Collection
            ...
            <NavLink
              to={PrivateRoutes.SUBMIT}
              className={({ isActive }: { isActive: boolean }) =>
                isActive
                  ? 'text-devYellow-400 font-medium underline decoration-2 underline-offset-4'
                  : 'text-slate-200 hover:text-white'
              }
            >
              Submit
          ...
          {user ? (
            ...
            <Link
              ...
              className='text-slate-200 hover:text-white font-primary bg-devPink-600 px-4 py-2 rounded-md font-bold hover:bg-devPink-500'
            >
              Login
            </Link>
          )}
          ...
          <div className='flex flex-col items-start justify-between h-full'>
            <nav className='flex flex-col h-full space-y-6 font-primary justify-center text-2xl'>
              <NavLink
                ...
                className={({ isActive }: { isActive: boolean }) =>
                  isActive
                    ? 'text-devPink-600 header-navLink-active'
                    : 'header-navLink'
                }
              >
                Explore
              ...
              <NavLink
                ...
                className={({ isActive }: { isActive: boolean }) =>
                  isActive
                    ? 'text-devBlue-600 header-navLink-active'
                    : 'text-slate-200 hover:text-white'
                }
              >
                Collection
              ...
              <NavLink
                ...
                className={({ isActive }: { isActive: boolean }) =>
                  isActive
                    ? 'text-devYellow-400 header-navLink-active'
                    : 'text-slate-200 hover:text-white'
                }
              >
                Submit
